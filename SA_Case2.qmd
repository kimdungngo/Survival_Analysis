...
simulate_case2 <- function(N, return_data = FALSE, include.immortal=FALSE) {
  N_grade <- floor(N / 6) 
  ...
  km_fit2 <- survfit(Surv(time, status) ~ group + grade, data = df2)
...
results_case2_total <- list()

for (N in n.obs) {
  data_N <- simulate_case2(N)
  print(data_N$plot)
  results_case2_total[[paste0("N_", N)]] <- list(
    data = data_N$data
  )
}
```
...
data_N <- simulate_case2(N = 1000, return_data=TRUE)  # charge N to any sample size
...
baseline_summary_2 <- list()

for (name in names(results_case2_total)) { #loop over each name in the list like N_100, N_150, etc
  data <- results_case2_total[[name]]$data #access data and assign for use inside the loop
  ...
  baseline_summary_2[[name]] <- summary_tbl
}
for (name in names(baseline_summary_2)) {
  cat("##### Sample Size:", name, "\n\n")
  print(baseline_summary_2[[name]])
  cat("\n\n")
}
```
...
set.seed(123)
data_1e6 <- simulate_case2(N = 1e6, return_data = TRUE)
```
...
for (N in n.obs) {
  l95s$l95s_correct[[as.character(N)]] <- sapply(seq_len(n_sim), function(i) {
    data_sim <- simulate_case2(N, return_data = TRUE)
...
l95s_sim <- l95s |>
  ...
  ggplot() + stat_pointinterval(aes(x=n.obs, y=l95, color=model), position = position_dodge(width=NULL))+ labs(x = "Sample Sizes", y = "Lower Bound")
```
...
-   Alternative hypothesis: l95 \> threshold (0.6): The lower bound of the 95% confidence interval for the treatment effect is at or below the threshold. The treatment then has less than 40% lower hazard than the c
